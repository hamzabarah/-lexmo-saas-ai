-- SUPERSEDED / DO NOT APPLY. Mission 005 uses 20260908_focus_v0.sql after the separate reset.
-- Focus execution v1. Apply BEFORE deploying the matching application.
-- No historical UPDATE/DELETE and no inferred historical timestamps.
BEGIN;
ALTER TABLE public.focus_sessions ADD COLUMN IF NOT EXISTS engine_version integer;
ALTER TABLE public.focus_sessions ALTER COLUMN engine_version SET DEFAULT 1;
...
-- Old duplicates (engine_version NULL) remain untouched. They block new starts.
CREATE UNIQUE INDEX IF NOT EXISTS focus_one_open_v1
 ON public.focus_sessions(user_id) WHERE engine_version = 1 AND ended_at IS NULL
...
BEGIN
 ...
 IF p_action = 'start' THEN
   ...
   INSERT INTO public.focus_sessions(user_id, task_id, subtask_id, task_title, category,
     planned_duration_minutes, started_at, status, engine_version)
   VALUES(p_user,t.id,(p_data->>'subtask_id')::uuid,t.title,t.category,planned,at_time,'running',1)
   RETURNING * INTO s;
   RETURN to_jsonb(s);
 END IF;
 sid := (p_data->>'id')::uuid;
 ...
 END IF;
 -- An old paused session has no trustworthy pause start. Require an explicit
 -- duration at closure; never infer it from historical updated_at.
 IF s.engine_version IS NULL AND s.status = 'paused' AND s.paused_at IS NULL AND
    NOT (p_action IN ('stop','abandon') AND p_data ? 'actual_minutes') THEN
   RAISE EXCEPTION 'focus_legacy_pause_requires_explicit_duration';
 END IF;
 ...
 ELSIF p_action IN ('stop','abandon','expire') THEN
   ...
   IF p_action = 'expire' AND (s.engine_version IS NULL OR s.status <> 'running' OR expired_at > at_time) THEN
     RAISE EXCEPTION 'focus_not_expirable';
   END IF;
   IF p_data ? 'actual_minutes' AND
      ((p_data->>'actual_minutes')::integer < 0 OR (p_data->>'actual_minutes')::integer > 480) THEN
     RAISE EXCEPTION 'focus_invalid_duration'; END IF;
