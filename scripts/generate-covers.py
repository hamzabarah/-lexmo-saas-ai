...
# -*- coding: utf-8 -*-
"""
Génération EN LOT des covers du blog ECOMY via l'API images d'OpenAI.

Lit content/editorial/covers.csv (colonnes : numero, slug, prompt) et génère
une image par ligne :
  - modèle gpt-image-2 (fallback gpt-image-1), qualité "high", paysage 16:9
  - recadrage final en 1280x720
  - enregistrement dans public/images/blog/covers/<slug>.png
  - si le PNG existe déjà -> saute (reprise possible)
  - 3 réessais par image, erreurs loguées dans covers-errors.log, on continue
...
Lancement :
    ...
    3) python scripts/generate-covers.py
"""
...
QUALITY = "high"
TARGET_W, TARGET_H = 1280, 720  # 16:9 final après recadrage (centré, inchangé)
MAX_RETRIES = 3

# Ajouté automatiquement à la FIN de chaque prompt du CSV au moment de l'appel API.
# Le recadrage retire le haut et le bas : on demande au modèle de garder tout le
# contenu important dans la zone centrale sûre avec de larges marges haut/bas.
FRAMING_SUFFIX = (
    " IMPORTANT framing: the image will be cropped to 16:9 by removing the top "
    "and bottom edges. Keep ALL text, badges and key elements fully inside the "
    "central safe zone, with generous empty margins at the very top and very "
    "bottom. Nothing important may touch the top or bottom edge."
)

ROOT = Path(__file__).resolve().parents[1]
CSV_FILE = ROOT / "content" / "editorial" / "covers.csv"
OUT_DIR = ROOT / "public" / "images" / "blog" / "covers"
ERROR_LOG = ROOT / "covers-errors.log"
...
def best_crop_top(img, crop_h: int) -> int:
    ...
    fenêtre de crop_h px MAXIMISE la luminosité contenue. Le texte/badges dorés
    sont lumineux, donc on garde la bande la plus « riche » en contenu doré.

    Repli sur le centre si numpy est absent."""
    w, h = img.size
    max_top = h - crop_h
    if max_top <= 0:
        return 0
    ...
    # Offsets de 0 à min(160, max_top) par pas de 8, en garantissant max_top.
...
def widen_to_ratio(img):
    ...
    # Fondu gauche : sur les 80px de marge collés à l'image, blend
    # médiane -> couleur de bord de l'image (alpha ~1 au raccord).
...
def main() -> int:
    ...
    try:
        ...
        print("❌ SDK manquant. Installe-le :  pip install openai pillow")
        return 1
    ...
    def generate(prompt: str):
        ...
        def call(model: str):
            return client.images.generate(
                model=model, prompt=full_prompt, size=API_SIZE, quality=QUALITY, n=1,
            )
        try:
